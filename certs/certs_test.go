package certs

import (
	"io/ioutil"
	"os"
	"path"
	"strings"
	"testing"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

var (
	testKey = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`

	testCert = `-----BEGIN CERTIFICATE-----

qO2mGEQNtAsgh5s827CAveJdU+FmdZu2cUIqkCtPGC2fURpGB9vN
-----END CERTIFICATE-----`
)

func TestGenerateSelfSigned(t *testing.T) {
	dirName, err := ioutil.TempDir("", "premkit-test")
	require.NoError(t, err)
	defer os.RemoveAll(dirName)

	keyFile, certFile, err := GenerateSelfSigned(dirName)
	require.NoError(t, err)

	// Read the files and confirm their contents
	keyData, err := ioutil.ReadFile(keyFile)
	require.NoError(t, err)
	certData, err := ioutil.ReadFile(certFile)
	require.NoError(t, err)

	assert.Equal(t, true, strings.HasPrefix(string(keyData), "-----BEGIN PRIVATE KEY-----"))
	assert.Equal(t, true, strings.HasPrefix(string(certData), "-----BEGIN CERTIFICATE-----"))

	// using the same directory, generate again to ensure the same key and cert is returned
	keyFile2, certFile2, err := GenerateSelfSigned(dirName)
	require.NoError(t, err)

	// Read the files and confirm their contents
	keyData2, err := ioutil.ReadFile(keyFile2)
	require.NoError(t, err)
	certData2, err := ioutil.ReadFile(certFile2)
	require.NoError(t, err)

	assert.Equal(t, keyData, keyData2)
	assert.Equal(t, certData, certData2)
}

func TestParseKeyPair(t *testing.T) {
	dirName, err := ioutil.TempDir("", "premkit-test")
	require.NoError(t, err)
	defer os.RemoveAll(dirName)

	err = ioutil.WriteFile(path.Join(dirName, "key"), []byte(testKey), 0400)
	require.NoError(t, err)
	err = ioutil.WriteFile(path.Join(dirName, "cert"), []byte(testCert), 0655)
	require.NoError(t, err)

	err = ParseKeyPair(path.Join(dirName, "key"), path.Join(dirName, "cert"))
	require.NoError(t, err)
}
